  ...
  moduleServer( id, function(input, output, session){
    ...
    observeEvent(ignoreInit = TRUE, ignoreNULL = TRUE, input$calage_validation, {
      ...
      ind_WarmUp_cal_cross1 <- seq(
        ...
        which(dataObsFirst()$date == dataObsFirst()$date[1]+(lubridate::years(options_de_mise_en_route$nbWarmUpYear())))
      )
      ...
      ind_WarmUp_val_cross1 <- seq(
        ...
        which(dataObsSecond()$date == dataObsSecond()$date[1]+(lubridate::years(options_de_mise_en_route$nbWarmUpYear())))
      )
      #* période d'exécution
    ...
    observeEvent(ignoreInit = TRUE, ignoreNULL = TRUE, input$simulation, {
      ...
      InputsModel_simulation <-  CreateInputsModel(
        FUN_MOD = airGR::RunModel_GR2M, DatesR = donnees$date,
      ...
      ind_WarmUp_simulation <-  seq(
        ...
        which(donnees$date == donnees$date[1]+(lubridate::years(options_de_mise_en_route$nbWarmUpYear())))
      )
      #* période d'exécution
