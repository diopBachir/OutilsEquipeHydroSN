...
mod_unique_periode_time_serie_interpolation_map_server <- function(id, bassin, stations, interpolationData){
  moduleServer( id, function(input, output, session){
    ...
    colorbarOptions$barTitleVjust <- -3
    ...
    colorbarOptions$barTitleMarginB <- .03
    ...
    observeEvent(input$colorbar, {
      showModal(modalDialog(
        ...
        fluidRow(
          ...
          column(4, sliderInput(ns("legTitleAngle"), label = "Orientation Du ttre De la Légende", min = 0, max = 360, value = colorbarOptions$legTitleAngle, step = .01, width = "100%")),
