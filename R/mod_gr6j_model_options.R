#' gr6j_model_options UI Function
#'
...
mod_gr6j_model_options_ui <- function(id){
  ns <- NS(id)
  tagList(
...
#' gr6j_model_options Server Functions
#'
#' @noRd
mod_gr6j_model_options_server <- function(id, ready_data_4_gr6j_application){
  moduleServer( id, function(input, output, session){
    ns <- session$ns

    # première période pour la validation croisé
    dataObs1<-reactive({
      shiny::req(ready_data_4_gr6j_application)
      ready_data_4_gr6j_application %>%
        slice(1:round(nrow(ready_data_4_gr6j_application)/2))
    })
...
# mod_gr6j_model_options_server("gr6j_model_options_1")
