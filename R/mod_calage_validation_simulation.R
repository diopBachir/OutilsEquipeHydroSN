#' calage_validation_simulation UI Function
#'
...
mod_calage_validation_simulation_ui <- function(id){
  ns <- NS(id)
  tagList(
...
#' calage_validation_simulation Server Functions
#'
#' @noRd
mod_calage_validation_simulation_server <- function(id, donnees, options_de_mise_en_route, parametres_du_modele){
  moduleServer(id, function(input, output, session){
    ...
    observeEvent(ignoreInit = TRUE, ignoreNULL = TRUE, input$calage_validation, {
      ...
      if(options_de_mise_en_route$nbWarmUpYear() >= nrow(dataObsFirst())){
        shinyalert::shinyalert(
          "Erreur !",
          "La période d'échauffement du modèle ne peut pas être supérieure ou égale à la période de Calibration.",
          type = "error"
        )
        testWarmup(FALSE)
      }
      ...
      InputsModel_cal_cross1 <- airGR::CreateInputsModel(
        FUN_MOD = airGR::RunModel_GR4J,        # modèle GR
      ...
      CalibOptions_cal_cross1 <- airGR::CreateCalibOptions(
        FUN_MOD = airGR::RunModel_GR4J,        # the name of the model function
      ...
      OutputsModel_cal_cross1 <- airGR::RunModel_GR4J(
      ...
      InputsModel_val_cross1 <-  CreateInputsModel(
        FUN_MOD = airGR::RunModel_GR4J, DatesR = dataObsSecond()$date,
      ...
      #------------------------------------------------------------------------------#
      ###* Run du modèle sur la période de calibration
      OutputsModel_cal_cross2 <- airGR::RunModel_GR4J(
      ...
      InputsModel_val_cross2 <-  CreateInputsModel(
        FUN_MOD = airGR::RunModel_GR4J, DatesR = dataObsFirst()$date,
      ...
      RunOptions_val_cross2 <- airGR::CreateRunOptions(
        FUN_MOD = airGR::RunModel_GR4J, InputsModel = InputsModel_val_cross2,
        IndPeriod_WarmUp = ind_WarmUp_val_cross2, IndPeriod_Run = ind_Run_val_cross2
      )

      #------------------------------------------------------------------------------#
      ###* Run du modèle sur la période de validation
      OutputsModel_val_cross2 <-  airGR::RunModel_GR4J(
    ...
    observeEvent(ignoreInit = TRUE, ignoreNULL = TRUE, input$simulation, {
      ...
      InputsModel_simulation <-  CreateInputsModel(
        FUN_MOD = airGR::RunModel_GR4J, DatesR = dataOverAll()$date,
      ...
      RunOptions_simulation <- airGR::CreateRunOptions(
        FUN_MOD = airGR::RunModel_GR4J, InputsModel = InputsModel_simulation,
        IndPeriod_WarmUp = ind_WarmUp_simulation, IndPeriod_Run = ind_Run_simulation
      )

      #------------------------------------------------------------------------------#
      ###* Run du modèle sur la période de validation
      OutputsModel_simulation <-  airGR::RunModel_GR4J(
    ...
    return(
      list(
        ...
        gr4j_simulation_parameters = reactive({ periode_simulation() })
      )
    )
...
# mod_calage_validation_simulation_server("calage_validation_simulation_1")
