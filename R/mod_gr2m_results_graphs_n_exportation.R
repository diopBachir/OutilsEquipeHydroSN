#' gr2m_results_graphs_n_exportation UI Function
#'
...
mod_gr2m_results_graphs_n_exportation_ui <- function(id){
  ns <- NS(id)
  tagList(
...
#' gr2m_results_graphs_n_exportation Server Functions
#'
#' @noRd
mod_gr2m_results_graphs_n_exportation_server <- function(
    # id
  id,
  ...
  moduleServer( id, function(input, output, session){
    ...
    observeEvent(ignoreInit = TRUE, ignoreNULL = TRUE, input$cross_validation_plot1, {
      ...
      ind_WarmUp_cal_cross1 <- seq(
        ...
        which(cross__validation_data_first$date == cross__validation_data_first$date[1]+(lubridate::years(warm__up_period)))
      )
      ...
      ind_WarmUp_val_cross1 <- seq(
        ...
        which(cross__validation_data_second$date == cross__validation_data_second$date[1]+(lubridate::years(warm__up_period)))
      )
      ...
      showModal(modalDialog(
        ...
        fluidRow(
          ...
          column(12, plotOutput(ns("plot_cross_validation")))
        ),
        footer=tagList(
          fluidRow(align = "center",
      ...
      output$toJPEG <-  downloadHandler(
        filename = function() {
          paste("Cross-Validation1--Calibration_Period-", stringr::str_replace_all(stringr::str_sub(Sys.time(), 1, 19), ":", "-"), ".jpeg")
        },
      ...
      output$toSVG <-  downloadHandler(
        filename = function() {
          paste("Cross-Validation1--Calibration_Period-", stringr::str_replace_all(stringr::str_sub(Sys.time(), 1, 19), ":", "-"), ".svg")
        },
        content = function(file) {
          # 1. Open jpeg file
      ...
      output$toExcel <-  downloadHandler(
        filename = function() {
          paste("Cross-Validation1--Calibration_Period-", stringr::str_replace_all(stringr::str_sub(Sys.time(), 1, 19), ":", "-"), ".xlsx")
        },
        content = function(file) {
          writexl::write_xlsx(
            list(
      ...
      observeEvent(ignoreNULL = TRUE, ignoreInit = TRUE, input$validation_plot, {
        ...
        output$toJPEG <-  downloadHandler(
          filename = function() {
            paste("Cross-Validation1--Validation_Period-", stringr::str_replace_all(stringr::str_sub(Sys.time(), 1, 19), ":", "-"), ".jpeg")
          },
          content = function(file) {
            # 1. Open jpeg file
            jpeg(file=file,   width = 13.3, height = 7.05, units = 'in', res = 300)
        ...
        output$toSVG <-  downloadHandler(
          filename = function() {
            paste("Cross-Validation1--Validation_Period-", stringr::str_replace_all(stringr::str_sub(Sys.time(), 1, 19), ":", "-"), ".svg")
          },
          content = function(file) {
            # 1. Open jpeg file
            svg(file=file,   width = 13.3, height = 7.05)
        ...
        output$toExcel <-  downloadHandler(
          filename = function() {
            paste("Cross-Validation1--Validation_Period-", stringr::str_replace_all(stringr::str_sub(Sys.time(), 1, 19), ":", "-"), ".xlsx")
          },
          content = function(file) {
            writexl::write_xlsx(
              list(
    ...
    observeEvent(ignoreInit = TRUE, ignoreNULL = TRUE, input$cross_validation_plot2, {
      ...
      showModal(modalDialog(
        ...
        fluidRow(
          ...
          column(12, plotOutput(ns("plot_cross_validation2")))
        ),
        footer=tagList(
          fluidRow(align = "center",
                   column(2, actionButton(
      ...
      output$toJPEG2 <-  downloadHandler(
        filename = function() {
          paste("Cross-Validation2--Calibration_Period-", stringr::str_replace_all(stringr::str_sub(Sys.time(), 1, 19), ":", "-"), ".jpeg")
        },
        content = function(file) {
          # 1. Open jpeg file
          jpeg(file=file,   width = 13.3, height = 7.05, units = 'in', res = 300)
      ...
      output$toSVG2 <-  downloadHandler(
        filename = function() {
          paste("Cross-Validation2--Calibration_Period-", stringr::str_replace_all(stringr::str_sub(Sys.time(), 1, 19), ":", "-"), ".svg")
        },
        content = function(file) {
          # 1. Open jpeg file
          svg(file=file,   width = 13.3, height = 7.05)
      ...
      output$toExcel2 <-  downloadHandler(
        filename = function() {
          paste("Cross-Validation2--Calibration_Period-", stringr::str_replace_all(stringr::str_sub(Sys.time(), 1, 19), ":", "-"), ".xlsx")
        },
        content = function(file) {
          writexl::write_xlsx(
            list(
      ...
      observeEvent(ignoreNULL = TRUE, ignoreInit = TRUE, input$validation_plot2, {
        ...
        output$toJPEG2 <-  downloadHandler(
          filename = function() {
            paste("Cross-Validation2--Validation_Period-", stringr::str_replace_all(stringr::str_sub(Sys.time(), 1, 19), ":", "-"), ".jpeg")
          },
          content = function(file) {
            # 1. Open jpeg file
            jpeg(file=file,   width = 13.3, height = 7.05, units = 'in', res = 300)
        ...
        output$toSVG2 <-  downloadHandler(
          filename = function() {
            paste("Cross-Validation2--Validation_Period-", stringr::str_replace_all(stringr::str_sub(Sys.time(), 1, 19), ":", "-"), ".svg")
          },
          content = function(file) {
            # 1. Open jpeg file
            svg(file=file,   width = 13.3, height = 7.05)
        ...
        output$toExcel2 <-  downloadHandler(
          filename = function() {
            paste("Cross-Validation2--Validation_Period-", stringr::str_replace_all(stringr::str_sub(Sys.time(), 1, 19), ":", "-"), ".xlsx")
          },
          content = function(file) {
            writexl::write_xlsx(
              list(
    ...
    observeEvent(ignoreInit = TRUE, ignoreNULL = TRUE, input$simulation_plot, {
      ...
      ind_WarmUp_simulation <- seq(
        ...
        which(overall__serie_data$date == overall__serie_data$date[1]+(lubridate::years(warm__up_period)))
      )
      ...
      output$toJPEG3 <-  downloadHandler(
        filename = function() {
          paste("Simulation-", stringr::str_replace_all(stringr::str_sub(Sys.time(), 1, 19), ":", "-"), ".jpeg")
        },
        content = function(file) {
          # 1. Open jpeg file
          jpeg(file=file,   width = 13.3, height = 7.05, units = 'in', res = 300)
      ...
      output$toSVG3 <-  downloadHandler(
        filename = function() {
          paste("Simulation-", stringr::str_replace_all(stringr::str_sub(Sys.time(), 1, 19), ":", "-"), ".svg")
        },
        content = function(file) {
          # 1. Open jpeg file
          svg(file=file,   width = 13.3, height = 7.05)
      ...
      output$toExcel3 <-  downloadHandler(
        filename = function() {
          paste("Simulation-", stringr::str_replace_all(stringr::str_sub(Sys.time(), 1, 19), ":", "-"), ".xlsx")
        },
        content = function(file) {
          writexl::write_xlsx(
            list(
    ...
    output$model_out<- renderDT({
      req(calibration_1, cross__validation_data_first)
...
# mod_gr2m_results_graphs_n_exportation_server("gr2m_results_graphs_n_exportation_1")
