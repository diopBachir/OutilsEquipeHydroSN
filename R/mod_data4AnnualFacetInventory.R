#' data4AnnualFacetInventory UI Function
#'
...
mod_data4AnnualFacetInventory_ui <- function(id){
  ns <- NS(id)
  tagList(
    fileInput(
      ns("dataInput"), label = div("Données A Inventorier", style = "color:#gray;family:Georgia;font-size:120%"),
...
#' data4AnnualFacetInventory Server Functions
#'
#' @noRd
mod_data4AnnualFacetInventory_server <- function(id){
  moduleServer( id, function(input, output, session){
    ...
    # loadingData
    data_for_annualFacetsInventoryGraph<- reactive({
      ...
      data_loaded<-reactive({
        ...
        testDateFormat<- reactive({
          ...
          for(feuille in readxl::excel_sheets(userFile()$datapath)){
            ...
            if(
              sum(grepl("[0-9]{4}", as.character(df.temp$Date))) != nrow(df.temp)
            ){
              dateFormatOK<-FALSE
              break
            }
        ...
        for(i in 1:length(readxl::excel_sheets(userFile()$datapath))){
          ...
          tempDF <- dplyr::select(tempDF, Facet, 1:(ncol(tempDF)-2)) %>%
            dplyr::mutate(
    ...
    output$used_data <- renderPrint({
      req(data_for_annualFacetsInventoryGraph())
      data_for_annualFacetsInventoryGraph()
    })

    #return
    return(
      list(
        data_for_annualFacetsInventoryGraph = reactive({data_for_annualFacetsInventoryGraph()}),
...
# mod_data4AnnualFacetInventory_server("data4AnnualFacetInventory_1")
