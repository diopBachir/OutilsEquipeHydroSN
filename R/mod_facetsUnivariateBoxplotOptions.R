...
mod_facetsUnivariateBoxplotOptions_ui <- function(id){
  ...
  tagList(
   ...
   fluidRow(
     ...
     column(6, sliderInput(ns("xAxisTextHjust"), label = div("X-axis H. Justification", style="family:Georgia;text-align:left;font-size:65%"), min=-5, max=5, step=.01, value=0.65, width="100%")),
     column(6, sliderInput(ns("xAxisTextVjust"), label = div("X-axis V. Justification", style="family:Georgia;text-align:left;font-size:65%"), min=-5, max=5, step=.01, value=.65, width="100%")),
     ...
     column(4, sliderInput(ns("axisTicksSize"), label = div("Ticks Size", style="family:Georgia;text-align:left;font-size:65%"), min=.1, max=7, step=.01, value = 1, width="100%")),
     column(4, sliderInput(ns("panelBackgroundLineSize"), label = div("Panel Size", style="family:Georgia;text-align:left;font-size:65%"), min=.5, max=7, step=.01, value=1.6, width="100%")),

     column(4, sliderInput(ns("outlierSize"), label = div("Outlier Size", style="family:Georgia;text-align:left;font-size:65%"), min=.2, max=10, step=.01, value=2., width="100%")),
     column(4, sliderInput(ns("boxplotsWidth"), label = div("Box. Width", style="family:Georgia;text-align:left;font-size:65%"),  min=.1, max=1, step=.01, value=.50, width="100%")),
     column(4, sliderInput(ns("boxplotAlpha"), label = div("Box. Alpha", style="family:Georgia;text-align:left;font-size:65%"), min=0, max=1, step=.1, value=1, width="100%")),

     column(4, sliderInput(ns("thresholdSize"), label = div("Seuil LineSize", style="family:Georgia;text-align:left;font-size:65%"), min=.2, max=10, step=.01, value=.9, width="100%")),
     column(4, sliderInput(ns("legendNrow"), label = div("Leg. Nrow", style="family:Georgia;text-align:left;font-size:65%"), min=1, max=10, value=1, step = 1, width="100%")),
     column(4, sliderInput(ns("legendKeyWidth"), label = div("Leg. KeyWidth", style="family:Georgia;text-align:left;font-size:65%"), min=.2, max=10, step=.01, value=.80, width="100%")),

     column(4, sliderInput(ns("legendKeyHeight"), label = div("Leg. KeyHeight", style="family:Georgia;text-align:left;font-size:65%"),  min=.2, max=10, step=.01, value=.8, width="100%")),
     column(4, sliderInput(ns("LegTextSize"), label = div("Leg. TextSize", style="family:Georgia;text-align:left;font-size:65%"),  min=5, max=35, step=1, value=13, width="100%")),
     column(4, sliderInput(ns("legTextMarginB"), label = div("Leg. TextMarg. B.", style="family:Georgia;text-align:left;font-size:65%"), min=-5, max=5, step=.01, value=0)),

     column(4, sliderInput(ns("legTextMarginT"), label = div("Leg. TextMarg. T.", style="family:Georgia;text-align:left;font-size:65%"), min=-5, max=5, step=.01, value=0)),
     column(4, sliderInput(ns("legTextMarginL"), label = div("Leg. TextMarg. L.", style="family:Georgia;text-align:left;font-size:65%"),  min=-5, max=5, step=.01, value=0)),
     column(4, sliderInput(ns("legTextMarginR"), label = div("Leg. TextMarg. R.", style="family:Georgia;text-align:left;font-size:65%"), min=-5, max=5, step=.01, value=0)),

     column(4, sliderInput(ns("legPosXcoord"), label = div("Leg. X-axis coords", style="family:Georgia;text-align:left;font-size:65%"),  min=0, max=1, step=.001, value=0.5)),
     column(4, sliderInput(ns("legPosYcoord"), label = div("Leg. Y-axis coords", style="family:Georgia;text-align:left;font-size:65%"),  min=0, max=1, step=.001, value=0.5)),
     column(4, sliderInput(ns("legMarginB"), label = div("Leg. Marg. B.", style="family:Georgia;text-align:left;font-size:65%"),min=-5, max=5, step=.01, value=0)),

     column(4, sliderInput(ns("legMarginT"), label = div("Leg. Marg. T.", style="family:Georgia;text-align:left;font-size:65%"),min=-5, max=5, step=.01, value=-.60)),
     column(4, sliderInput(ns("legMarginL"), label = div("Leg. Marg. L.", style="family:Georgia;text-align:left;font-size:65%"), min=-5, max=5, step=.01, value=0)),
     column(4, sliderInput(ns("legMarginR"), label = div("Leg. Marg. R.", style="family:Georgia;text-align:left;font-size:65%"), min=-5, max=5, step=.01, value=0)),

     column(4, sliderInput(ns("facetNcol"), label = div("Facets Nrow", style="family:Georgia;text-align:left;font-size:65%"), min=2, max=12, step=1, value=2)),
     column(4, sliderInput(ns("stripTextSize"), label = div("Strip TextSize", style="family:Georgia;text-align:left;font-size:65%"),  min=5, max=35, step=1, value=13)),
     column(4, sliderInput(ns("stripTextMarginB"), label = div("Strip Text Marg. B.", style="family:Georgia;text-align:left;font-size:65%"), min=-5, max=5, step=.01, value=2)),

     column(4, sliderInput(ns("stripTextMarginT"), label = div("Strip Text Marg. T.", style="family:Georgia;text-align:left;font-size:65%"), min=-5, max=5, step=.01, value=0)),
     column(4, sliderInput(ns("stripTextMarginL"), label = div("Strip Text Marg. L.", style="family:Georgia;text-align:left;font-size:65%"), min=-5, max=5, step=.01, value=0)),
     column(4, sliderInput(ns("stripTextMarginR"), label = div("Strip Text Marg. R.", style="family:Georgia;text-align:left;font-size:65%"), min=-5, max=5, step=.01, value=0)),

     column(4, sliderInput(ns("panelYspacing"), label = div("panelYspacing", style="family:Georgia;text-align:left;font-size:65%"),min=-5, max=5, step=.01, value=.5)),
     column(4, sliderInput(ns("panelXspacing"), label = div("panelXspacing", style="family:Georgia;text-align:left;font-size:65%"),min=0, max=5, step=.01, value=-.01)),
   ...
   fluidRow(
     ...
     column(4, selectInput(ns("legDir"), label = div("Leg. direction", style="family:Georgia;text-align:left;font-size:65%"),  choices = c("H"="horizontal", "V"="vertical"), selected = "H")),
     ...
     column(4, selectInput(ns("FacetScale"), label = div("FacetScales", style="family:Georgia;text-align:left;font-size:65%"),  choices = c("fixed", "free", "free_x", "free_y"), selected = "free")),
     column(8, textAreaInput(ns("variableColors"), label = div("Couleur des Groupes", style="family:Georgia;text-align:left;font-size:65%"), value = "", placeholder = "red;green;blue;cyan;orange;...")),
